import { insertNodesBatch } from './sqlite.js';
/**
 * 根据 roots, depth, childrenPerNode 计算理论总节点数
 * 公式：nodes = roots × (c^d - 1) / (c - 1)
 */
export function computeTotalNodes(roots, maxDepth, childrenPerNode) {
 ...
 * 生成大型树形数据（森林）
 */
export function generateLargeTree(options) {
    ...
    return currentId;
    function generateSubtree(parentId, currentDepth, parentPath) {
 ...
 * 生成平衡树
 */
export function generateBalancedTree(depth, branchingFactor) {
    let totalNodes = 0;
    const batch = [];
    const batchSize = 1000;
    function flushBatch() {
    ...
    function createLevel(parentId, currentDepth, parentPath) {
