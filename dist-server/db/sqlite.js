...
export function insertNode(node) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
    const result = stmt.run(node.parentId, node.name, node.level, node.path, node.hasChildren ? 1 : 0, node.isLeaf ? 1 : 0, node.sortOrder);
    return result.lastInsertRowid;
}
/**
 * 批量插入节点（使用事务）
 */
export function insertNodesBatch(nodes) {
  ...
  `);
    const insertMany = db.transaction((nodesList) => {
        ...
        for (const node of nodesList) {
            const result = insertStmt.run(node.parentId, node.name, node.level, node.path, node.hasChildren ? 1 : 0, node.isLeaf ? 1 : 0, node.sortOrder);
 ...
 * 获取根节点
 */
export function getRootNodes(limit = 100, offset = 0) {
 ...
 * 获取子节点
 */
export function getChildren(parentId, limit = 100, offset = 0) {
    const rows = db.prepare(`
    SELECT id, parent_id as parentId, name, level, path,
           has_children as hasChildren, is_leaf as isLeaf, sort_order as sortOrder
    FROM tree_nodes
 ...
 * 获取可见节点（递归查询）
 */
export function getVisibleNodes(limit = 100, offset = 0) {
    const sql = `
...
export function getNodeCount() {
    const result = db.prepare('SELECT COUNT(*) as count FROM tree_nodes').get();
    return result.count;
}
/**
 * 搜索节点
 */
export function searchNodes(keyword, limit = 50) {
    const sql = `
    SELECT id, parent_id as parentId, name, level, path,
           has_children as hasChildren, is_leaf as isLeaf
    FROM tree_nodes
    ...
    LIMIT ?
  `;
    const rows = db.prepare(sql).all(`%${keyword}%`, limit);
    return rows.map(row => ({
        id: row.id,
        parentId: row.parentId,
        name: row.name,
